/************************************************************************
*                                                                       *
* NuiImpl.cpp -- cpp file of the callback function implementation       *
*                                                                       *
...
// lookup-tables for color tinting based on player index
...
const int DEPTH_HIGHT = 240;
const int COLOR_WIDTH = 640;
const int COLOR_HIGHT = 480;
const int RECORD_TIME_INTVAL = 500; // [ms]

// extension of output images
const std::string FILE_EXT = ".bmp";
std::string colorOutputFile = "./record/colorImgRec";
std::string depthOutputFile = "./record/depthImgRec";
//const std::string PATH_BASE = "../record";

// global variables
cv::Mat depthImage;
cv::Mat colorImage;
cv::Mat colorResizedImage;

int currentColorImageIndex = 1;
int currentDepthImageIndex = 1;
BOOL startDepth = false;
// write video file
//cv::VideoWriter ivWriter;
...
void CSkeletalViewerApp::Nui_GotColorAlert( )
{
    ...
    if ( LockedRect.Pitch != 0 )
    {
		...
		// store color imagen in opencv container and resize it
		colorImage.create(COLOR_HIGHT, COLOR_WIDTH, CV_8UC4);
		colorImage.data = static_cast<BYTE *>(LockedRect.pBits);
		colorResizedImage.create(DEPTH_HIGHT, DEPTH_WIDTH, CV_8UC4);
		cv::resize(colorImage, colorResizedImage, cvSize(DEPTH_WIDTH, DEPTH_HIGHT));
		
		// save resized color image into individual image file
		// compose the output file name
		/*std::stringstream colorImageFileName;
		colorImageFileName << colorOutputFile << std::setfill('0') 
			<< std::setw(6) << currentColorImageIndex++ << FILE_EXT;
		cv::imwrite(colorImageFileName.str(), colorResizedImage);*/
		
		// Once per second, save a image if we get a depth image
		if(startDepth == true)
		{
			std::stringstream colorImageFileName;
			colorImageFileName << colorOutputFile << std::setfill('0') 
			...
			colorTimer = timeGetTime( );
			if ( (colorTimer - m_LastColorSaveTime) > RECORD_TIME_INTVAL )
			{
				cv::imwrite(colorImageFileName.str(), colorResizedImage);
				m_LastColorSaveTime = colorTimer;
				currentColorImageIndex++;
				
			}	
		}

		// write image into video file
		/*ivWriter.open("../record/color.avi", CV_FOURCC('M', 'J', 'P', 'G'), 25, cvSize(DEPTH_WIDTH, DEPTH_HIGHT), true);
		ivWriter.write(colorResizedImage);*/
		
		// debugging purpose
		/*cv::namedWindow("Color Image");
		cv::imshow("Color Image", colorResizedImage);
		cv::waitKey(1);*/
    }
    else
    {
...
void CSkeletalViewerApp::Nui_GotDepthAlert( )
{
    ...
    if ( 0 != LockedRect.Pitch )
    {
        ...
        // draw the bits to the bitmap (RGB) 320x240
        RGBQUAD * rgbrun = m_rgbWk;
		// draw the bit to intensity map (grayscale) 320x240
		BYTE * ingrun = m_ingBody;
        ...
        while ( pBufferRun < pBufferEnd )
        {
			...
			*ingrun = Nui_ShortToQuadifBody_Depth( *pBufferRun );
            ++pBufferRun;
            ++rgbrun;
			++ingrun;
        }
		...
		// store depth imagen in opencv container	
		depthImage.create(DEPTH_HIGHT, DEPTH_WIDTH, CV_8UC1);
		depthImage.data = (BYTE *)m_ingBody;
		
		// save depth image into individual image file
		// compose the output file name
		/*std::stringstream depthImageFileName;
		depthImageFileName << depthOutputFile << std::setfill('0') 
			<< std::setw(6) << currentDepthImageIndex++ << FILE_EXT;
		cv::imwrite(depthImageFileName.str(), depthImage);*/

		// Once per second, save a image
		if(startDepth == true)
		{
			std::stringstream depthImageFileName;
			depthImageFileName << depthOutputFile << std::setfill('0') 
			...
			depthTimer = timeGetTime( );
			if ( (depthTimer - m_LastDepthSaveTime) > RECORD_TIME_INTVAL )
			{
				cv::imwrite(depthImageFileName.str(), depthImage);
				m_LastDepthSaveTime = depthTimer;
				currentDepthImageIndex++;
...
// Function to show skeleton background
...
void CSkeletalViewerApp::Nui_GotSkeletonAlert( )
{
	...
	// 20.02.2012: use new variable skeletal_enable
	if (skeletal_enable == true)
	{
	// Nui_BlankSkeletonScreen( GetDlgItem( m_hWnd, IDC_SKELETALVIEWER ), false );
    Nui_BlankSkeletonScreen( GetDlgItem( m_hWnd, IDC_DEPTHVIEWER ), false );
    ...
    for ( int i = 0 ; i < NUI_SKELETON_COUNT; i++ )
    {
        ...
        {
            Nui_DrawSkeleton( &SkeletonFrame.SkeletonData[i], GetDlgItem( m_hWnd, IDC_DEPTHVIEWER ), i );
        }
        // Tracking function deactivated
		/*else if ( m_bAppTracking && SkeletonFrame.SkeletonData[i].eTrackingState == NUI_SKELETON_POSITION_ONLY )
        {
            Nui_DrawSkeletonId( &SkeletonFrame.SkeletonData[i], GetDlgItem( m_hWnd, IDC_DEPTHVIEWER ), i );
        }*/
    }
    ...
    Nui_DoDoubleBuffer(GetDlgItem(m_hWnd,IDC_DEPTHVIEWER), m_SkeletonDC);
	}
}
...
BYTE CSkeletalViewerApp::Nui_ShortToQuadifBody_Depth( USHORT s )
{
    USHORT RealDepth = NuiDepthPixelToDepth(s);
    USHORT Player    = NuiDepthPixelToPlayerIndex(s);
	...
	if ( Player != 0 )
	{
		intensity = intensity >> g_IntensityShiftByPlayerB[Player];
		startDepth = true;
	}
	else
	{
...
// 20.02.2012: deactivated the corresponding callback function of AppTracking
